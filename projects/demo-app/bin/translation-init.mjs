...
function patchPackageJsonScripts (pkgJson, mode, projects) {
  for (const [name, config] of projects) {
    const isSingle = !projects.length && config.root === ''
    ...
    const out = isSingle
      ? 'src/assets/i18n'
      : `projects/${name}/src/assets/i18n`
    const scriptPath = isSingle
      ? `node ./bin/translation-build.mjs`
      : `node projects/${name}/bin/translation-build.mjs`

    if (mode === 'ts-to-json') {
      pkgJson.scripts[prebuildKey] = `${scriptPath} --src=${src} --out=${out}`
      pkgJson.scripts[startKey] = `npm run ${prebuildKey} && ng serve${
        isSingle ? '' : ` ${name}`
      }`
    } else {
      // Classic mode: just serve; no prebuild needed
      pkgJson.scripts[prebuildKey] =
        'echo "No build step needed for JSON-only mode."'
...
async function main () {
  ...
  const angularConfig = loadJson(
    angularJsonPath,
    '[i18n:init] angular.json not found.'
  )
  const projects = angularConfig.projects || {}
  const appProjects = Object.entries(projects).filter(
    ([_, config]) => config.projectType === 'application'
  )
  ...
  if (i18nMode === 'json-only') {
    ...
    writeIfNotExist(
      path.join(assetsDir, 'zh-Hant.json'),
      `{\n  "hello": "你好，世界！"\n}`
    )
    console.log(
      '[i18n:init] ✅ Sample JSON translation files created at src/assets/i18n/'
    )
...
`
    ...
    console.log(
      '[i18n:init] ✅ Sample TypeScript translation file created at src/i18n/en/index.ts'
    )
    console.log(
      '[i18n:init] ⚠️  translation-build.mjs stub created at bin/translation-build.mjs. Implement your own TS→JSON logic!'
    )
  }
